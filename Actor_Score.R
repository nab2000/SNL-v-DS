Actor_Score <- function (dir = "./", save_name = "Actor_Score", 
                     load_movie = "movie_list_RT", load_file = "Actor_List.csv", 
    ...
    for (d in 1:nrow(Actor_df)) {
        sub <- movie_df$Score[movie_df$Actor == Actor_df$Actor[d],]
        Actor_Score[d,1] <- Actor_df$Actor[d]
        Actor_Score[d,1] <- Actor_df$BegYear[d]
        Actor_Score[d,1] <- Actor_df$EndYear[d]
        Actor_Score[d,1] <- Actor_df$Show[d]
    ...
    names(Actor_Score) <- c("Actor", "BegYear", "EndYear", "Show", "Mean", "Med", "Max", "SD")
